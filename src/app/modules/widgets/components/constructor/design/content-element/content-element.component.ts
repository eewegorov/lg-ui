...
export class ContentElementComponent implements OnInit, DoCheck, AfterContentChecked {
  ...
  @Output() private scrollToElement = new EventEmitter<{
    element: string;
    index: number | string;
  ...
  public addExtraClassForFullWidth(index) {
    ...
    const indexValueForm = this.widget.guiprops.formExt.enable
      ? $('.margin-if-form-ext-element').css('z-index')
      : $('.margin-if-form-element').css('z-index');
  ...
  public getItemClassData(item) {
    if (
      item.name === 'title-element' ||
      item.name === 'image-element' ||
      item.name === 'iframe-element' ||
      item.name === 'video-element' ||
      item.name === 'padding-element' ||
      item.name === 'split-element' ||
      item.name === 'coupon-element'
    ) {
  ...
  public classNameImg() {
    return this.widgetConstructorService.classNameImg(
      this.widget.guiprops.image,
      this.widget.guiprops.form,
      this.widget.guiprops.formExt?.model?.mainSettings
    );
  }
  ...
  private getColorPodBorderRadius() {
    if (
      this.widget.guiprops.formExt?.model?.mainSettings?.visual?.type === 1 ||
      this.widget.guiprops.form.visual === 'На всю ширину' ||
      this.widget.guiprops.image.enable === false
    ) {
      if (
        this.widget.guiprops.image.place === 'Слева' ||
        this.widget.guiprops.image.place === 'Справа' ||
        this.widget.guiprops.image.place === 'Сверху' ||
