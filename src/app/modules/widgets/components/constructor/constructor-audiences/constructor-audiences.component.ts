  ...
  public removeItem(groupId, index) {
    for (let i = 0; i < this.widget.audience.groups.length; i++) {
      if (this.widget.audience.groups[i].id === groupId) {
        this.widget.audience.groups[i].items = this.widgetConstructorService.removeFromArray(
          this.widget.audience.groups[i].items,
          index
        );
      ...
      if (item.id === groupId) {
        for (let j = 0; j < item.items.length; j++) {
          if (item.items[j].type === itemType) {
            item.items[j].subitems = this.widgetConstructorService.removeFromArray(item.items[j].subitems, index);
