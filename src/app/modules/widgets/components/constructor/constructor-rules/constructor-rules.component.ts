...
export class ConstructorRulesComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.widgetSettingsCategories = this.widgetSettingsCategories.filter(
      (_, index) => !this.isContainerized || (this.isContainerized && index !== 2 && index !== 3)
    );
  }
  ...
  public removePrevPageRuleItem(index) {
    this.widget.rules.prevPages.items = this.widgetConstructorService.removeFromArray(
      this.widget.rules.prevPages.items,
      index
    );
  }
  ...
  private validator() {
    ...
    if (
      this.widget.autoresponder?.enabled &&
      (!this.widget.autoresponder?.subject?.toString().trim().length ||
        !this.widget.autoresponder?.text?.toString().trim().length)
    ) {
      errors.push({
        id: TAB_ID,
      ...
      for (let i = 0; i < this.widget.audience?.groups?.length; i++) {
        for (let j = 0; j < this.widget.audience?.groups?.[i]?.items?.length; j++) {
          for (let k = 0; k < this.widget.audience?.groups?.[i]?.items?.[j]?.subitems?.length; k++) {
            if (
              (this.widget.audience?.groups?.[i]?.items?.[j].type === 'REFER' ||
