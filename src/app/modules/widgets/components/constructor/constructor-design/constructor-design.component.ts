...
export class ConstructorDesignComponent implements OnInit, AfterViewInit, OnDestroy, OnChanges {
  ...
  public placeLabel = [
    ...
    '',
    '',
    'Левая сторона браузера',
    '',
    'Правая сторона браузера',
    'Нижний левый угол',
    '',
    'Нижний правый угол'
  ];
    ...
    this.autoUploadSub = this.flow?.events$.subscribe(event => {
      if (event.type === 'filesSubmitted' && event?.event[0][0]) {
        if (event.event[0][0].file.size > 2000000) {
          ...
          if (
            event.event[0][0].file.type !== 'image/jpeg' &&
            event.event[0][0].file.type !== 'image/png' &&
            event.event[0][0].file.type !== 'image/gif'
          ) {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes && changes.widget && !changes.widget.isFirstChange()) {
      ...
      this.widgetType = this.widgetService
        ...
        .find((item: WidgetType) => item.id === this.widget.type).code;
      this.staticWidgetInstallCode = this.isContainerized
        ? this.containerizedWidgetService.getContainerInstallCode(this.widget.containerId)
        : '';
  ...
  ngOnInit(): void {
    ...
    if (
      ...
      this.widget.guiprops?.bg.fillorImg === 'useVideo' &&
      this.widget.guiprops?.bg.video.videoId
    ) {
      this.newVideoSize(this.widget.guiprops.bg.video);
    }

    if (
      this.widget.guiprops.image.enable &&
      this.widget.guiprops.image.typeBl &&
      this.widget.guiprops.image.typeBl === 'videoBl'
    ) {
    ...
    this.globalCouponObject = {
      ...
      title: {
        enable: false,
        textSummer:
          '<p>Вы можете редактировать этот текст. Если вы хотите<br>изменить цвет, позиционирование или стиль текста,<br>то выделите фрагмент для появления окна редактора.<br>Размер и шрифт изменяются слева в блоке настроек элемента.</p>',
        font: this.systemFonts[0],
        fontType: 'systemFont',
        fontName: '',
        fontSize: 12,
  ...
  public removeElementFromFormExtList(event, index) {
    ...
    this.widget.guiprops.formExt.model.list = this.widgetConstructorService.removeFromArray(
  ...
  public isElementHasError(elementName: string, elementCounter: number) {
    ...
    for (const item of errors) {
      if (
        ...
        ((item.element === elementName && item.counter === elementCounter) ||
          (!item.counter && item.element === elementName))
      ) {
        return true;
      }
    }
  ...
  public isThankShouldShow() {
    ...
    return (
      (this.widget.guiprops.form?.enable && !this.widget.guiprops.formSet?.redirect.enable) ||
  ...
  public scrollToEl(element: string, index: number | string, elementCounter?: number) {
    ...
    if (
      ...
      if (index) {
        ...
        if (
          target.offset().top - accordion.offset().top - 42 <= 10 &&
          target.offset().top - accordion.offset().top - 42 >= -10
        ) {
          return;
        }
  ...
  public buildIframeElement(item, index: number, rebuild: boolean): void {
    setTimeout(() => {
      ...
      ifrm.setAttribute(
        'style',
        'position:relative!important;width:100%!important;height:100%!important;border:none!important'
      );
      ...
      doc
        .open()
        .write(
          '<body>' + '<style>body{margin:0!important;}' + item.css_value + '</style>' + item.html_value + '</body>'
        );
      doc.close();
    }, 0);
  }
  ...
  public getVideoId(item) {
    ...
    if (item.videoType === 'youtube') {
      ...
      if (result) {
        ...
        result =
          result +
          '?controls=0&iv_load_policy=3&showinfo=0&rel=0' +
          (item.isVideoBG || item.autoplay ? '&autoplay=1' : '') +
          '&loop=1&mute=1&modestbranding=1&disablekb=1&playlist=' +
          result;
  ...
  public newVideoSize(item, rebuild?: boolean) {
    setTimeout(() => {
      ...
      if (item.width_type) {
        ...
        $('#idVideoFrame' + item.counter).css({
          height: $('#idVideoFrame' + item.counter).innerWidth() / 1.666 + 'px'
        });
      }
    }, 0);
  }
  ...
  private validator() {
    ...
    this.widget.guiprops.elementsList.forEach(el => {
      ...
      if (
        el.name === 'social-element' &&
        this.widget.guiprops['social'].linkForShare === 'own' &&
        !this.widget.guiprops['social'].linkText
      ) {
        errors.push({
          id: TAB_ID,
          element: el.name,
  ...
  private loadListener() {
    ...
    if (
      typeof this.widget.guiprops.labelMain.iconOrImage === 'undefined' &&
      this.widget.guiprops.labelMain.icon.enable
    ) {
      ...
      setTimeout(() => {
        if (!$('#thankWidget').hasClass('active')) {
          if (this.widget.guiprops.image.enable) {
            ...
            if (
              this.widgetConstructorService.ruleLeftOrRightUnderContent(
                this.widget.guiprops.formExt,
            ...
            if (
              this.widgetConstructorService.ruleLeftOrRightWholeWidth(
                this.widget.guiprops.formExt,
                this.widget.guiprops.form.visual,
                this.widget.guiprops.image.place
              )
            ...
            if (
              !this.widget.guiprops.button?.enable &&
              !this.widget.guiprops.form?.enable &&
              !this.widget.guiprops.formExt?.enable &&
  ...
  private checkChanges(): void {
    ...
    if (this.widget.guiprops.formExt) {
      if (this.widget.guiprops.formExt.model.mainSettings.colorPod.enable) {
        this.widget.guiprops.formExt.model.mainSettings.colorPod.rgbaColorPod = this.widgetConstructorService
          .hexToRgb(
    ...
    if (this.widget.guiprops.form.colorPod.enable) {
      this.widget.guiprops.form.colorPod.rgbaColorPod = this.widgetConstructorService
        .hexToRgb(this.widget.guiprops.form.colorPod.color, this.widget.guiprops.form.colorPod.opacityColorPod)
        .toString();
    } else {
    ...
    if (this.widget.guiprops.form.bgInputForm && this.widget.guiprops.form.opacityBgInputForm) {
      this.widget.guiprops.form.rgbaInputForm = this.widgetConstructorService
        .hexToRgb(this.widget.guiprops.form.bgInputForm, this.widget.guiprops.form.opacityBgInputForm)
        .toString();
    }
    ...
    if (this.widget.guiprops.popupMain.shadow.enable) {
      this.widget.guiprops.popupMain.shadow.rgbaColor = this.widgetConstructorService
        .hexToRgb(this.widget.guiprops.popupMain.shadow.color, this.widget.guiprops.popupMain.shadow.opacityColor)
        .toString();
    } else {
    ...
    if (this.widget.guiprops.labelMain.colorLabel != null) {
      this.widget.guiprops.labelMain.rgbaLabel = this.widgetConstructorService
        .hexToRgb(this.widget.guiprops.labelMain.colorLabel, this.widget.guiprops.labelMain.opacityBgLabel)
        .toString();
    }

    if (this.widget.guiprops.dhVisual.colorBg) {
      this.widget.guiprops.dhVisual.rgbaShadowForm1 = this.widgetConstructorService
        .hexToRgb(this.widget.guiprops.dhVisual.colorBg, 0.6)
        .toString();
      this.widget.guiprops.dhVisual.rgbaShadowForm2 = this.widgetConstructorService
        .hexToRgb(this.widget.guiprops.dhVisual.colorBg, 0.8)
        .toString();
    }
    ...
    if (this.widget.guiprops.dhVisual.widget_content_width === 'Собственная') {
      ...
      if (
        this.widget.guiprops.image.enable &&
        (this.widget.guiprops.image.place === 'Слева' || this.widget.guiprops.image.place === 'Справа')
      ) {
    ...
    if (this.widget.guiprops.bg.shadow) {
      if (!this.widget.guiprops.bg.shadow.enable) {
        ...
        this.widget.guiprops.bg.shadow.style =
          this.widget.guiprops.bg.shadow.horiz +
          'px ' +
          this.widget.guiprops.bg.shadow.vertical +
          'px ' +
          this.widget.guiprops.bg.shadow.blur +
          'px ' +
          this.widgetConstructorService.getRGBAColor(this.widget.guiprops.bg.shadow);
      }
    }

    if (this.widget.guiprops.bg.mask.enable) {
      this.widget.guiprops.bg.mask.rgbaColor = this.widgetConstructorService
        .hexToRgb(this.widget.guiprops.bg.mask.color, this.widget.guiprops.bg.mask.opacity)
        .toString();
    } else {
    ...
    if (this.widget.guiprops.form.enable) {
      ...
      for (let i = 0; i < this.widget.guiprops.formSet.items.length; i++) {
        if (
          this.widget.guiprops.formSet.items[i].type === 'message' ||
          this.widget.guiprops.formSet.items[i].type === 'phone'
        ) {
  ...
  private fillListImage(images: Image[]) {
    ...
    for (const item of images) {
      ...
      $('#listImagesBlock').append(
        '<div class="imageGlItem" data-link="' +
          link +
          '" data-id="' +
          name +
          '"><div class="imageGlItemIn"><div class="imageItem" style="background: url(' +
          link +
          ') center no-repeat; background-size:cover"></div></div><div class="delete-cur-img-btn" data-del="' +
          name +
          '"><span></span></div></div>'
      );
    }
  ...
  private downUpInitAir() {
    ...
    globalAir.find('.note-btn-group').each(function () {
  ...
  private downUpInit() {
    ...
    globalBl.find('.dropdown').each(function () {
      const topInner = $(this).offset().top;
      if (topInner >= topH - 250) {
        $(this).addClass('dropup');
      } else {
    ...
    globalBl.find('.font-select').each(function () {
      const topInner = $(this).offset().top;
      if (topInner >= topH - 250) {
        $(this).addClass('dropup');
      } else {
